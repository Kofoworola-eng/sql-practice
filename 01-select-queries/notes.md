...
`GROUP BY` collapses multiple rows into a summary row per group —
used together with aggregate functions like `SUM`, `AVG`, `COUNT`,
`MIN`, `MAX`.
...
`WHERE`, but for grouped/aggregated data rather than individual rows.
...
`TOP` becomes meaningful when combined with `ORDER BY` — this lets
you get the "top N" by an actual ranking, like highest or lowest
score:
...
in `ORDER BY`, but not in `WHERE` — because `WHERE` runs before
`SELECT` even exists yet, but `ORDER BY` runs after.
...
- `TOP` needs `ORDER BY` to be meaningful — otherwise "top" is
  arbitrary
- `DISTINCT` removes duplicate rows/values from the result
- Coding order and execution order are different — understanding
  ...
  later on (e.g. aliases, nested filtering)